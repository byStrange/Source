...
if (window === undefined) {
    ...
    function setVariable() {
        ...
        if (attributes.class) {
            ...
            for (let c = 0; c < className.length; c++) {
                ...
                window[`$_${className[c]}s`] = document.querySelectorAll(`.${className[c]}`);
            }
        }
    }
    setVariable()
    ...
    HTMLElement.prototype.on = function event(op, cx) {
        try {
            if (is.fnc(op.click)) {
    ...
    HTMLElement.prototype.frontdelete = function frontdelete() {
        if (this instanceof HTMLInputElement || this instanceof HTMLTextAreaElement) {
            this.value = this.value.slice(this.value.length - this.value.length + 1, this.value.length);
        } else {
            this.innerText = this.textContent.slice(this.textContent.length - this.textContent.length + 1, this.textContent.length);
        }
    };
    ...
    Window.prototype.on = function event(op, cx) {
        ...
        } else if (is.num(op) || is.num(cx) || is.str(cx) || is.arr(cx) || is.obj(cx)) {
            error('First argument should be event and second one must be function ');
        }
        if (this == null || op == undefined) {
    ...
    var temp = this
    if (safe) {
        customElements.define(temp.name, class extends HTMLElement {
            connectedCallback() {
